...
function mapToFountain(db) {
  return {
    ...
    isOsm: !!db.is_osm,
    amenity: db.amenity || "drinking_water"
  };
}
...
function findNearestEuropeanCityServer(lat, lng) {
  const referenceCities = [
    ...
    { name: "Caltanissetta", lat: 37.4903, lng: 14.062 },
    { name: "Agrigento", lat: 37.3111, lng: 13.5765 },
    { name: "Trapani", lat: 38.0178, lng: 12.515 },
    { name: "Cagliari", lat: 39.2238, lng: 9.1217 },
    { name: "Sassari", lat: 40.7259, lng: 8.5556 },
    { name: "Alghero", lat: 40.558, lng: 8.3181 },
    ...
    { name: "Salerno", lat: 40.678, lng: 14.7881 },
    ...
    { name: "Sanremo", lat: 43.816, lng: 7.7738 },
    ...
    { name: "Alessandria", lat: 44.9129, lng: 8.615 },
...
(
  ...
  area["name"="M\xFCnchen"]["admin_level"="8"]->.o;
...
out body;`;
  ...
  try {
    ...
    if (res.ok) {
      ...
      const elementFeatures = elements.filter((el) => el.type === "node").map((el) => {
    ...
    if (supabase) {
      try {
        ...
        if (latMin && latMax && lngMin && lngMax) {
          osmQuery = osmQuery.gte("lat", Number(latMin)).lte("lat", Number(latMax)).gte("lng", Number(lngMin)).lte("lng", Number(lngMax));
        } else {
          osmQuery = osmQuery.limit(1e3);
        }
